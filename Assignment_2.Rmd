...
```{r fit diagnostics region fig.width = 100}
...
```{r fit diagnostics 3 countries}
country_model <-  model_2_aug %>% filter(country_name == c("Senegal", "Peru", "Australia"))%>% rowid_to_column( "ID")
...
ggplot(data = country_model, aes(x = ID, y = .cooksd))+geom_col()+
  facet_wrap(~ country_name, scales = "free_x") +
  theme_bw() 
```
